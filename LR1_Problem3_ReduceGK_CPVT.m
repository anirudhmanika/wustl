...
[~,~,~,~, ~, ~, vars.finitMUTATED, ~] = ISIfMUTATED(Vrest);
...
%mutated
currentsMUTATED = [0 0 0 0 0 0];
outL(1,:)=[v0 Ca0];
vars = varsSave;
for i = 1:length(t)-1
    %Midpoint
    [eval1, ~,currentsMUTATED] = luoRudySolverMUTATED(t(i), outL(i,:), vars, i, currentsMUTATED);
    k1 = vars.dt .* eval1;
    
    [eval2, vars,currentsMUTATED] = luoRudySolverMUTATED(t(i) + 0.5 * vars.dt, outL(i,:) + 0.5 .* k1,vars, i, currentsMUTATED);
...
figure;
sgtitle(['LR1 Model + 50% Reduction of gK in Ik eqn ', num2str(max(vars.stim)), '\muA/cm^2 Injection for ',num2str(inj1(3)-inj1(2)), 'ms @ t=', num2str(inj1(2)), 'ms'], FontSize=15)

%voltage
subplot(2,1,1);
plot(t,v, LineWidth=1.5);
hold on;
plot(t,vL, LineWidth=1.5);
hold off;
legend("Normal V_m","V_m with 0.5*gK", FontSize=13)
...
ylabel("Inj. Curr. (\muA/cm^2)", FontSize=14)
grid on;

xlabel("Time (ms)", FontSize=15)

% %was just checking the current to see it
% figure;
% INaL = currentsLQT3(2,:);
% plot(t, INaL);
% legend("inaL",FontSize=13)
% ylabel("current (\muA/cm^2)", FontSize=14)
% xlabel("time")
% grid on;
...
%Luo-Rudy Derivatives Output Func. (MUTATED Phenotype)
function [dXdt, vars,currents] = luoRudySolverMUTATED(t,x, vars, index,currents)
    Iinj = interp1(vars.t, vars.stim, t, 'nearest', 'extrap');
    ...
    [~,~,~,~,~,~, mss, taum, hss, tauh, jss, tauj] = INAf(v);
    [~,~,~,~, dss, taud, fss, tauf] = ISIfMUTATED(v);
    [Xi,~,~, Xss, tauX] = IKf(v);
    [~, ~, K1ss, ~] = IK1f(v,vars);
    Kp = IKPf(v);
    ...
    j = jss + (vars.jinit - jss) * exp(-(ddt) / tauj);
    d = dss + (vars.dinit - dss) * exp(-(ddt)/ taud);
    f = fss + (vars.finitMUTATED - fss) * exp(-(ddt) / tauf); 
    X = Xss + (vars.Xinit - Xss) * exp(-(ddt) / tauX);
    vars.minit =m;vars.hinit=h;vars.jinit=j; vars.dinit=d;vars.finitMUTATED =f;vars.Xinit=X;
    
    %currents
    INa(index) = vars.gbarNa * m^3 * h * j *(v-vars.ENa);
    Isi(index) = 0.09 * d * f * (v-Esi);
    Ik(index)= 0.50 * vars.gbarK * X * Xi * (v-vars.Ek);
    IK1(index) = vars.gbarK1 * K1ss * (v-vars.Ek1);
    IKP(index) = vars.gbarKP * Kp * (v-vars.Ekp);
    Ib(index) = vars.gbarB * (v-vars.Eb);
