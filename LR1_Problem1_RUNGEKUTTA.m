...
for i = 1:length(t)-1
    %Runge Kutta
    %see testodes_inclass.m on the QP Canvas for this process

    [eval1, ~,currents] = luoRudySolver(t(i), out(i,:), vars,...
        i, currents);

    k1 = vars.dt .* eval1;

    [eval2, ~, currents] = luoRudySolver(t(i) + 0.5 * vars.dt, out(i,:) + 0.5 .* k1,...
        vars, i, currents);

    k2 = vars.dt .* eval2;
    %this is where we ended for midpoint, but RK uses 4 midpoints as a
    %weighted avg

    [eval3, ~,currents] = luoRudySolver(t(i) + 0.5 * vars.dt, out(i,:) + 0.5 .* k2,...
        vars, i, currents);

    k3 = vars.dt .* eval3;

    [eval4, vars, currents] = luoRudySolver(t(i), out(i,:) + k3,...
        vars, i, currents);

    k4 = vars.dt .* eval4;

    out(i+1,:) = out(i,:) + k1/6 + k2/3 + k3/3 + k4/6;
...
figure;
sgtitle(['(Runge-Kutta, dt=0.26) LR1 Model Response to ', num2str(max(vars.stim)), '\muA/cm^2 Injection for ',num2str(inj1(3)-inj1(2)), 'ms @ t=', num2str(inj1(2)), 'ms'], FontSize=15)
